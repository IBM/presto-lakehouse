...
- [Set up an Open Lakehouse](#set-up-an-open-lakehouse)
  ...
  - [3. Connect Presto to Table Formats](#3-connect-presto-to-table-formats)

## 1. Build the minimal Hive metastore image

In order to use our table formats with Presto, we will to set up a Hive metastore to sync our data across different database engines. Hudi integration with Presto currently requires a Hive metastore, but there are other catalog options when using Iceberg with Presto including Hive, Nessie, Glue, and REST server. You can read more about the options on the [Iceberg Connector page](https://prestodb.io/docs/current/connector/iceberg.html#metastores) of the Presto docs.

For this lab, we'll use the Hive metastore for all our table format integrations and build a minimal Hive metastore image from the Dockerfile included in this repo to do so.
...
## 3. Connect Presto to Table Formats

Our containers are up and running, but you may be wondering how Presto works with our tables formats, as we didn't see any special key-value pairs for this in the docker compose file. Presto makes it very easy to get started with table formats, with no need to install any additional packages. If we started the Presto CLI right now, we would be able to query any existing tables synced to the Hive metastore in Hudi or Iceberg format - but how? Recall the volume that we passed to the `coordinator` container. This volume includes a directory called `catalog` that was mapped to the `/opt/presto-server/etc/catalog` location in the container along with the other server configuration files. The `catalog` directory is where the Presto server looks to see what underlying data sources should be made available to Presto and how to connect to those sources. Let's take a look at the `iceberg.properties` file that was mapped to the Presto cluster.
...
And similarly, the `hudi.properties` file:
